...
func (repo *articleRepository) ListPub(ctx context.Context, now time.Time, offset int, size int) ([]domain.Article, error) {
	res, err := repo.dao.ListPub(ctx, now, offset, size)
	if err != nil {
		return nil, err
	}
